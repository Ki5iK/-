...
while True:
    try:
        ISIN = input(
            'Введите ISIN облигации по которой требуется произвести расчёт купонной доходности: ').upper().strip()
        URL_ISIN = 'http://bonds.finam.ru/issue/search/default.asp?emitterCustomName=' + ISIN
        Res_URL_ISIN = requests.get(URL_ISIN)
        Code_Finam = re.search(
            r'details(\w{5})\/default.asp', Res_URL_ISIN.text).group(1)
        break
    except:
...
try:
    Bonds_Pays = [(datetime.datetime.strptime(re.search(
        r'(\d{2}\.\d{2}\.\d{4})', Res_URL_Pays.text).group(0), '%d.%m.%Y').date(), '', '', '', 0, 0)]
    Bonds_Pays += re.findall(r'(\d{2}\.\d{2}\.\d{4})&nbsp;<\/font><\/td><td align=right>'r'([^&]*)&nbsp;<\/td><td align=right>([^&]*)&nbsp;&nbsp;<\/td><td align=right>'
                            r'([^&]*)&nbsp;[^>]*><td align=right>'
...
Bond_Nominal = int(re.findall('Номинал:&nbsp;<span>(\d{0,3}\s\d*)</span>&nbsp;', Res_URL_Pays.text)[0].replace('\xa0', '').replace(' ', ''))

# Парсим страницу оферт, если она есть. В противном случае будет одна дата оферты = дате погашения.
URL_Offers = 'http://bonds.finam.ru/issue/details' + \
    Code_Finam + '00003/default.asp'
Res_URL_Offers = requests.get(URL_Offers)

try:
    Bonds_Offers = re.findall(
        r'(\d{2}\.\d{2}\.\d{4})[^100]*100%', Res_URL_Offers.text)
...
for i in range(1, len(Bonds_Pays)):
    ...
    if Bonds_Pays[i][1]:
        Bonds_Pays[i][1] = float(
            Bonds_Pays[i][1].replace(',', '.').replace('%', ''))
    else:
...
for i in range(len(Bonds_Offers)):
    Bonds_Offers[i] = datetime.datetime.strptime(
        Bonds_Offers[i], '%d.%m.%Y').date()
...
while True:
    try:
        Bond_Bay_Percent = float(
            (input('Задайте цену покупки в %')).replace(',', '.')) / 100
        break
    except:
...
for i in range(len(Bonds_Pays) - 1):
    ...
    if Bonds_Pays[i][0] <= Day_Bay < Bonds_Pays[i + 1][0]:
        NKD = Bond_Nominal_Current * \
            Bonds_Pays[i + 1][1] / 100 * \
            ((Day_Bay - Bonds_Pays[i][0]).days / 365)
        break

# Определяем полную цену покупки
print('При покупке облигации мы заплатим:' + str(Bond_Nominal_Current *
    Bond_Bay_Percent) + ' + НКД: ' + str(NKD), end='')
print('+ комиссия брокера' +
      str((Bond_Nominal_Current * Bond_Bay_Percent) * Commission_Broker))
Bond_Bay = Bond_Nominal_Current * Bond_Bay_Percent + NKD + \
    (Bond_Nominal_Current * Bond_Bay_Percent) * Commission_Broker
...
while True:
    try:
        ...
        Day_Say = datetime.datetime.strptime(
            input('Введите дату продажи в формате DD.MM.YYYY:'), '%d.%m.%Y').date()
...
for i in range(len(Bonds_Pays) - 1):
    ...
    if Bonds_Pays[i][0] <= Day_Bay < Bonds_Pays[i + 1][0] and Bonds_Pays[i][0] < Day_Say:
        ...
        print('NKD', NKD)
        Money_Back.append(
            (Bonds_Pays[i + 1][0], round(NKD + Bonds_Pays[i + 1][-1], 2)))
        Day_Bay = Bonds_Pays[i + 1][0]
    elif Day_Bay >= Day_Say:
        NKD = Bond_Nominal * Bonds_Pays[i + 1][1] / 100 * (
            (Day_Say - Bonds_Pays[i][0]).days / 365)
        Money_Back[-1] = (Day_Say, round(NKD + Money_Back[-1]
                        [1] + Bond_Nominal_Current, 2))
        break
...
print('Доходность к погашению полная = ' +
      str(round(xirr(Money_Back) * 100, 2)) + '%')
